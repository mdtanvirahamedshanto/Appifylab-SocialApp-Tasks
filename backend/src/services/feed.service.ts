...
const findTargetOrThrow = async (target: LikeTarget) => {
  if (target.type === LikeTypeValues.POST) {
    const post = await prisma.post.findUnique({ where: { id: target.targetId }, select: getTargetSelect(target) });
    if (!post) throw new AppError(404, "Post not found");
    return post;
  }

  if (target.type === LikeTypeValues.COMMENT) {
    const comment = await prisma.comment.findUnique({ where: { id: target.targetId }, select: getTargetSelect(target) });
    if (!comment) throw new AppError(404, "Comment not found");
    return comment;
  }

  const reply = await prisma.reply.findUnique({ where: { id: target.targetId }, select: getTargetSelect(target) });
