...
interface FeedPostCardProps {
  ...
  onStartReplyToComment: (postId: string, commentId: string, mentionName?: string) => void;
}
                  ...
                  <div className="_comment_area">
                    ...
                    {comment.replies.map((reply) => {
                      if (isTempEntityId(reply.id)) {
                        return (
                          <div className="feed-skeleton-row _mar_t16 _mar_l16" key={reply.id} aria-label="Loading reply">
                            <div className="feed-skeleton feed-skeleton-avatar" />
                              ...
                              <div className="_comment_reply">
                                <div className="_comment_reply_num">
                                  <ul className="_comment_reply_list">
                                    <li>
                                      <span
                                        style={{ cursor: "pointer", opacity: !!actionBusy[`like:REPLY:${reply.id}`] || isTempEntityId(reply.id) ? 0.6 : 1 }}
                                        onClick={() => {
                                          if (!!actionBusy[`like:REPLY:${reply.id}`] || isTempEntityId(reply.id)) return;
                                          onToggleLike("REPLY", reply.id, post.id);
                                        }}
                                      >
                                        {reply.viewerLiked ? "Unlike." : "Like."}
                                      </span>
                                    </li>
                                    <li>
                                      <span
                                        ...
                                        onClick={() => {
                                          onStartReplyToComment(post.id, comment.id, toName(reply.author));
                                        }}
                                      >
                                        Reply.
                                      </span>
                    ...
                    {openReplyBoxes[replyKey] ? (
                      <div className="_feed_inner_comment_box _mar_t12">
                        <form
                          className="_feed_inner_comment_box_form"
                          onSubmit={(event) => {
                            event.preventDefault();
